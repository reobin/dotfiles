...
- After changing Herdr config, run `tt "$(cat ~/.config/tt/current)"`. Herdr never reads `config.base.toml`; re-applying the current theme regenerates `config.toml` from it, then reloads the server.
